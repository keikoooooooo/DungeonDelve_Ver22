...
public class RewardManager : Singleton<RewardManager>
{
    ...
    private readonly Dictionary<RewardSetup.ItemReward, int> _itemRewardsData = new();
    ...
    /// </summary>
    /// <param name="_itemReward"> Thông tin phần thưởng. </param>
    public void AddNoticeReward(RewardSetup.ItemReward _itemReward)
    {
        var _val = _itemReward.GetValue();
        ...
        if (_itemRewardsData.ContainsKey(_itemReward)) return;
        
        // Tạo thông báo có phần thưởng
        RewardNoticeManager.CreateNoticeT2(_des, _sprite);

        // Thêm dữ liệu phần thưởng vào danh sách chờ
        _itemRewardsData.Add(_itemReward, _val);
    }
    
    /// <summary>
    /// Xóa thông báo phần thưởng trên UI khi người dùng không còn đứng gần vật phẩm
    /// </summary>
    /// <param name="_itemReward"></param>
    public void RemoveNoticeReward(RewardSetup.ItemReward _itemReward)
    {
        if(!_itemRewardsData.ContainsKey(_itemReward)) return;

        _itemRewardsData.Remove(_itemReward);
        RewardNoticeManager.ReleaseAllNoticeT2();
        
        foreach (var keyValuePair in _itemRewardsData)
        {
            var _des = keyValuePair.Key.GetDescription();
            var _nameCode = keyValuePair.Key.GetNameCode();
            Sprite _sprite = null;
            if (gameItemData.GetItemCustom(_nameCode, out var itemCustom))
            {
                _sprite = itemCustom.sprite;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.P) && _itemRewardsData.Count >= 1)
        {
            foreach (var (key, value) in _itemRewardsData)
            {
                Debug.Log("key = " + key.GetNameCode());
                Debug.Log("val = " + value);
                break;
            }
        }
